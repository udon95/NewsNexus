...
const ManageRooms = () => {
  ...
  const [newPublicRoom, setNewPublicRoom] = useState({ name: "", description: "" });
  const [newPrivateRoom, setNewPrivateRoom] = useState({ name: "", description: "", invite: "" });
  ...
  const handleAddPublicRoom = async () => {
    const res = await fetch("http://localhost:5000/api/rooms", {
      ...
      body: JSON.stringify({ ...newPublicRoom, privacy: "Public", created_by: userId }),
    });
  ...
  const handleAddPrivateRoom = async () => {
    const res = await fetch("http://localhost:5000/api/rooms", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...newPrivateRoom, privacy: "Private", created_by: userId }),
    });
    ...
    if (res.ok) {
      ...
      for (let username of usernames) {
        await fetch("http://localhost:5000/api/rooms/invite", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            inviter_id: userId,
  ...
  const handleAcceptInvite = async (roomid) => {
    await fetch("http://localhost:5000/api/rooms/accept", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const handleExitRoom = async (roomid) => {
    await fetch("http://localhost:5000/api/rooms/exit", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userid: userId, roomid }),
    });
