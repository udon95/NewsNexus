  ...
  export const PremiumWriteArticle = () => {
    ...
    useEffect(() => {
      ...
      if (postType === "General") {
        ...
        pendingImages.forEach((img) => {
          editorHtml = editorHtml.replace(new RegExp(`<img src="${img.previewUrl}"[^>]*>`, 'g'), '');
        });
  ...
  if (postType === "General") {
    ...
    if (postid) {
      // Now upload images for room article
      for (const img of pendingImages) {
        const file = img.file;
        if (!file?.name) continue;
    ...
    const handleEditorImageUpload = (e) => {
      ...
      const validFiles = files.filter((file) => {
        if (file.size > 50 * 1024 * 1024) {
          alert("One of the images exceeds the 50MB limit. Please upload smaller files.");
          return false;
        }
        return true;
      });
      ...
      validFiles.forEach((file) => {
        ...
        // Check postType before accumulating editor images
        if (postType === "General") {
          insertImages += `<img src="${previewUrl}" /><p></p>`; 
        }
      });
    
      // Only insert images into editor if postType is General
      if (postType === "General" && insertImages) {
        editor.chain().focus().insertContent(insertImages).run();
      }
                  ...
                  {pendingImages.map((img, index) => (
                    <div key={index} className="relative border rounded-lg overflow-hidden shadow">
                      ...
                      <button
                        ...
                        const updatedHtml = editorHtml.replace(new RegExp(`<img src="${targetUrl}"[^>]*>`, 'g'), '');
