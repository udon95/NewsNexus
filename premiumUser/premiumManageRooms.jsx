...
const ManageRooms = () => {
  const [publicRooms, setPublicRooms] = useState([{ id: 1, name: "Lee Hsien Long" }]);
  ...
  const [existingRoomNames, setExistingRoomNames] = useState(["Lee Hsien Long"]); // Prevent duplicate names
      ...
      <div className="w-full max-w-3xl bg-white p-10 rounded-2xl shadow-lg relative">
        ...
        {publicRooms.map((room) => (
          <div key={room.id} className="flex items-center mb-2">
            <input type="text" value={room.name} readOnly className="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            <button onClick={() => handleEditRoom(room, "public")} className="ml-2 px-3 py-1 bg-gray-700 text-white rounded-lg">Update</button>
            <button onClick={() => handleDeleteRoom(room.id, "public")} className="ml-2 px-3 py-1 bg-gray-700 text-white rounded-lg">Delete</button>
            <button className="ml-2 px-3 py-1 bg-gray-700 text-white rounded-lg">Exit</button>
          </div>
        ))}
        ...
        {privateRooms.map((room) => (
          <div key={room.id} className="flex items-center mb-2">
            <input type="text" value={room.name} readOnly className="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            <button onClick={() => handleEditRoom(room, "private")} className="ml-2 px-3 py-1 bg-gray-700 text-white rounded-lg">Update</button>
            <button onClick={() => handleDeleteRoom(room.id, "private")} className="ml-2 px-3 py-1 bg-gray-700 text-white rounded-lg">Delete</button>
            <button className="ml-2 px-3 py-1 bg-gray-700 text-white rounded-lg">Exit</button>
          </div>
        ))}
      ...
      {isCreating && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-lg bg-white p-6 rounded-lg shadow-lg">
            ...
            <select value={newRoom.userLimit} onChange={(e) => setNewRoom({ ...newRoom, userLimit: e.target.value })} className="w-full p-2 border rounded-lg mb-2">
              <option value="20">20 Users</option>
              <option value="50">50 Users</option>
              <option value="100">100 Users</option>
            </select>

            <textarea placeholder="Room Description" value={newRoom.description} onChange={(e) => setNewRoom({ ...newRoom, description: e.target.value })} className="w-full p-2 border rounded-lg mb-2 resize-none" rows="3" />

            <input type="text" placeholder="Category" value={newRoom.category} onChange={(e) => setNewRoom({ ...newRoom, category: e.target.value })} className="w-full p-2 border rounded-lg mb-2" />

            <select value={newRoom.privacy} onChange={(e) => setNewRoom({ ...newRoom, privacy: e.target.value })} className="w-full p-2 border rounded-lg mb-2">
              <option value="public">Public</option>
              <option value="private">Private</option>
            </select>

            <button onClick={handleSaveRoom} className="w-full p-2 mt-2 bg-blue-500 text-white rounded-lg">{editingRoom ? "Update" : "Create"} Room</button>
