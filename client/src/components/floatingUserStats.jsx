...
const FloatingUserStats = ({ user, customClassName }) => {
  const [stats, setStats] = useState({ articles: 0, drafts: 0, comments: 0, reads: 0 });
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        const [articlesRes, draftsRes, commentsRes, readsRes] = await Promise.all([
          supabase
            .from("articles")
  ...
  ? supabase
      ...
      .select("*", { count: "exact", head: true })
      .eq("userid", userId)
      .gte("read_date", `${new Date().toISOString().split("T")[0]}T00:00:00Z`)
      .lte("read_date", `${new Date().toISOString().split("T")[0]}T23:59:59Z`)
  : supabase
      .from("reading_history")
      .select("*", { count: "exact", head: true })
      .eq("userid", userId)
  ...
  return (
    ...
    <div className={`mt-[35px] z-40 flex flex-col items-start gap-3 ${customClassName || "top-[498px] left-20"}`}>
