...
const AdminCommunityNotes = () => {
  ...
  useEffect(() => {
    setDisplayedArticlesList(articles.filter((article) => (pendingNotes.some((note) => note.target_id === article.articleid) && !approvedNotes.some((note) => note.target_id === article.articleid))));
  ...
  useEffect(() => {
    ...
    setSelecteddArticlesNotes(notes.filter((note) => note.target_id===selectedArticle.articleid));
  }, [selectedArticle]);
  ...
  const handleResolvedStatusChange = () => {
    ...
    setDisplayedArticlesList(articles.filter((article) => statusElement=="Approved"? approvedNotes.some((note) => note.target_id === article.articleid) : (pendingNotes.some((note) => note.target_id === article.articleid) && !approvedNotes.some((note) => note.target_id === article.articleid))));
  };
  ...
  const setDisplayNotes = async (row) => {
    ...
    if (error) {
      console.error("Error fetching data:", error);
        ...
        alert(row.Status=="Pending"?"Community note set!":"Community note removed");
        ...
        <div className="flex-1 font-grotesk">
          {selectedArticle ? (
            <div>
              ...
              <div className="ml-10 mt-5 max-w-[500px] bg-gray-100 rounded-2xl p-3 text-lg shadow-lg outline-none focus:ring-2 focus:ring-gray-300">
                Article Status : &emsp;
                {selectedArticle && selectedArticle.Suspended ? "Suspended" : "Active"}
              </div>
