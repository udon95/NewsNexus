  ...
  export const PremiumEditArticle = () => {
    ...
    const handlePostGeneralArticle = async () => {
      ...
      // 🧹 Delete old draft images from article_images table
      await supabase
      .from("article_images")
      .delete()
      .eq("articleid", id);
      ...
      for (const img of pendingImages) {
        ...
        if (urlData?.publicUrl) {
          ...
          doc.querySelectorAll("img").forEach((imgTag) => {
            if (img.previewUrl && imgTag.getAttribute("src") === img.previewUrl) {
              imgTag.setAttribute("src", urlData.publicUrl);
            }
          });
          updatedHTML = doc.body.innerHTML;          
    ...
    const handlePostRoomArticle = async () => {
      ...
      //   if (pathError) {
      //     console.error("Error updating room imagepath:", pathError);
      //   }
      // }
    ...
    return (
      //DEVI MADE CHANGES HERE SO THAT THE DISPLAY NOT SKEWED
