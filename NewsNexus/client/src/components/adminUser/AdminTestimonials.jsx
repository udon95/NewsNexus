...
const AdminTestimonials = () => {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      setDisplayedRows(rows.filter((row) => row.homepage_display === displayedStatus));
    ...
    <div className="w-screen min-h-screen flex flex-col overflow-auto">
      <div className="flex">
        ...
        {selectedItem ? (<div>          
            ...
            <div className="ml-10 mt-5 max-w-[500px] bg-gray-100 rounded-2xl p-3 break-words text-lg shadow-lg outline-none focus:ring-2 focus:ring-gray-300">
              Shared Experience : <br />{selectedItem ? selectedItem.share_experience : ""}
            </div>
            <div className="ml-10 mt-5 max-w-[500px] bg-gray-100 rounded-2xl p-3 break-words text-lg shadow-lg outline-none focus:ring-2 focus:ring-gray-300">
             Areas of Improvement : <br />{selectedItem ? selectedItem.areas_to_improve : ""}
            </div>
            <div className="flex">
              <button
                type="button"
                className="px-6 py-3 bg-[#3F414C] flex ml-10 mt-7 text-white rounded-lg hover:bg-opacity-90 cursor-pointer"            
                onClick={() => setDisplay(selectedItem.id,(selectedItem.homepage_display?false:true))}>
                {selectedItem.homepage_display ?"Takedown testimonial" : "Display testimonial"}
              </button>
            </div>
          </div>): <div></div>}
          <div className="flex">
